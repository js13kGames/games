  ...
  init: function () {
    ...
    document.body.addEventListener('mousedown', () => {
      if (!gunAnim) {
        ...
        if (bullets <= 0) {
          el.emit("reload");
          setTimeout(() => {bullets = 7;}, 1000); // and here
        }
      }
    });
  }
...
// This script is ran after gunEvents.js to wait for the elements to be created in HTML
window.onload = function(){
  var anim = document.getElementById("gun");
  anim.addEventListener("animationstart", () => {gunAnim = true}, false);
  // I used a workaround here because animationend was ran before the actual end.
  anim.addEventListener("animationend", ()=>{
  if(init) {
    setTimeout(()=>{gunAnim = false}, 700);
    init = false;
  } else {
    gunAnim = false;
  }}, false);
